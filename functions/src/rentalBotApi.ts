...
app.post("/overview", async (request, response) => {
  await handle(response, async () => {
    ...
    const openCharges = data.charges.filter((charge) => ["pending", "overdue", "in_review"].includes(String(charge.status ?? "")));
    const overdueCharges = openCharges.filter((charge) => String(charge.status ?? "") === "overdue");
    const pendingTotal = openCharges.reduce((sum, charge) => sum + Number(charge.total ?? 0), 0);
    ...
    return {
      ...
      whatsappMessage: [
        ...
        overdueCharges.length ? `Morosos: ${buildTenantNames(overdueCharges, data.tenants)}` : "No hay cobros vencidos."
      ].join("\n")
    };
  });
});
...
app.post("/tenantStatus", async (request, response) => {
  await handle(response, async () => {
    const scope = requireScope(request.body?.ownerScope);
    const query = String(request.body?.query ?? "").trim().toLocaleLowerCase("es-AR");
    if (!query) {
    ...
    const tenant = data.tenants.find((item) =>
      String(item.fullName ?? item.name ?? "").toLocaleLowerCase("es-AR").includes(query)
    );
    if (!tenant) {
      throw new ApiError(404, "No encontré ese inquilino dentro de tus departamentos.");
    }

    const property = data.properties.find((item) => item.id === String(tenant.propertyId ?? ""));
    ...
    return {
      ...
      whatsappMessage: [
        ...
        current ? `Período: ${String(current.period ?? "sin período")}` : "Sin cobros registrados.",
        current ? `Estado: ${humanizeChargeStatus(current.status)}` : "",
        current ? `Total: ${formatCurrency(Number(current.total ?? 0))}` : "",
        current?.dueDate ? `Vencimiento: ${String(current.dueDate)}` : "",
        Number(current?.overdueDays ?? 0) > 0 ? `Atraso: ${Number(current?.overdueDays)} días` : ""
      ].filter(Boolean).join("\n")
    };
  });
});
...
app.post("/sendWarnings", async (request, response) => {
  await handle(response, async () => {
    ...
    const channel = String(settingsDoc.get("defaultNotificationChannel") ?? "email") as "auto" | "whatsapp" | "sms" | "email";
    const openCharges = data.charges.filter((charge) => ["pending", "overdue"].includes(String(charge.status ?? "")));
    let sent = 0;
    let failed = 0;

    for (const charge of openCharges) {
      const overdue = String(charge.status ?? "") === "overdue";
...
async function loadScopedRentalData(scope: OwnerScope) {
  const [propertiesSnapshot, tenantsSnapshot, chargesSnapshot] = await Promise.all([
  ...
  const properties = propertiesSnapshot.docs
    .map(mapDoc)
    .filter((property) => scope === "all" || resolvePropertyScope(property) === scope);
  const propertyIds = new Set(properties.map((property) => property.id));
  const tenants = tenantsSnapshot.docs.map(mapDoc).filter((tenant) => propertyIds.has(String(tenant.propertyId ?? "")));
  const tenantIds = new Set(tenants.map((tenant) => tenant.id));
  const charges = chargesSnapshot.docs.map(mapDoc).filter((charge) =>
    propertyIds.has(String(charge.propertyId ?? "")) || tenantIds.has(String(charge.tenantId ?? ""))
  );
  return {properties, tenants, charges};
}

function mapDoc(document: FirebaseFirestore.QueryDocumentSnapshot) {
  return {id: document.id, ...document.data()} as Record<string, unknown> & {id: string};
}
...
function buildTenantNames(charges: Array<Record<string, unknown>>, tenants: Array<Record<string, unknown> & {id: string}>) {
  const names = new Set(charges.map((charge) => {
    const tenant = tenants.find((item) => item.id === String(charge.tenantId ?? ""));
...
async function handle(response: Response, operation: () => Promise<Record<string, unknown>>) {
  try {
    ...
    response.status(status).json({
      ok: false,
      error: status === 500 ? "Error interno." : error instanceof Error ? error.message : "Error interno."
    });
  }
}
