...
import { backendBaseUrl, mercadoPagoAccessToken, webAppUrl } from "../config.js";
import { processTransferPaymentSubmission } from "./payments.js";

function applyCors(response: { set: (name: string, value: string) => unknown }, origin?: string | null) {
  const configuredOrigin = new URL(webAppUrl.value()).origin;
  ...
  const safeOrigin = origin && allowedOrigins.has(origin) ? origin : configuredOrigin;
...
export const resolvePaymentAccessToken = onRequest(async (request, response) => {
  ...
  const tokenDoc = await db.collection("paymentAccessTokens").doc(token).get();
  if (!tokenDoc.exists) {
  ...
  if (tokenData.status !== "active" || new Date(tokenData.expiresAt) < new Date()) {
    response.status(410).json({ ok: false, error: "token_expired" });
    return;
  }

  const chargeDoc = await db.collection("charges").doc(String(tokenData.chargeId)).get();
  if (!chargeDoc.exists) {
...
export const createCheckoutFromPaymentAccessToken = onRequest(async (request, response) => {
  applyCors(response, request.headers.origin);
  ...
  if (!accessToken || !apiBaseUrl) {
    response.status(500).json({ ok: false, error: "provider_not_configured" });
    return;
  }
  ...
  const preferencePayload = {
    ...
    back_urls: {
      success: `${webAppUrl.value()}/?token=${token}&mp_status=success`,
      failure: `${webAppUrl.value()}/?token=${token}&mp_status=failure`,
      pending: `${webAppUrl.value()}/?token=${token}&mp_status=pending`
    },
  ...
  if (!preferenceResponse.ok) {
    ...
    response.status(502).json({ ok: false, error: "mercado_pago_preference_failed", detail: errorBody });
    return;
  }
...
export const submitTransferFromPaymentAccessToken = onRequest(async (request, response) => {
  applyCors(response, request.headers.origin);
  ...
  try {
    ...
    const status = error instanceof HttpsError ? mapHttpsStatus(error.code) : 500;
    response.status(status).json({ ok: false, error: "transfer_validation_failed", detail: message });
  }
});
...
async function saveTokenReceipt(input: {
  ...
  if (!input.file.dataBase64) {
    throw new HttpsError("invalid-argument", "Uno de los comprobantes llego vacio.");
  }

  const fileBuffer = decodeBase64File(input.file.dataBase64);
  if (!fileBuffer.length) {
    throw new HttpsError("invalid-argument", "Uno de los comprobantes no pudo procesarse.");
  }

  if (fileBuffer.length > 6 * 1024 * 1024) {
    throw new HttpsError("invalid-argument", "Cada comprobante debe pesar menos de 6 MB.");
  }
  ...
  await storage.bucket().file(storagePath).save(fileBuffer, {
