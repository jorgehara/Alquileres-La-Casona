...
- [x] 4.2 Verify callable Function access for the matrix across representative endpoints exported from `functions/src/index.ts`: admin dataset, tenant onboarding/claiming, payments/receipts, settings/bootstrap, and audit-sensitive operations. Evidence: `npm run verify:auth-authority` passed with callable checks for `getScopedAdminDataset`, `claimTenantAccess`, `submitTransferPayment`, `upsertGeneralSettings`, and `writeAuditLog`, including stale, disabled, missing-profile, and unauthenticated denials.
- [x] 4.3 Verify `firestore.rules` matrix access for representative collections: `users`, `properties`, `tenants`, `charges`, `payments`, `paymentReceipts`, `rentReceipts`, `messages`, and `tenantInvitations`. Evidence: `npm run verify:auth-authority` passed Firestore emulator REST checks across all listed collections with tenant own/other, admin owner-scope, disabled, missing-profile, stale-claim, and unauthenticated cases.
- [x] 4.4 Verify `storage.rules` matrix access for `utility-bills`, `payment-receipts`, `contracts`, `message-attachments`, and `rent-receipts` paths for the current Storage claim-projection model. Evidence: `npm run verify:auth-authority` passed Storage emulator REST read/write allow/deny checks on all listed paths. Limitation: Storage rules cannot read canonical `users/{uid}` profiles, so this proves token-projection behavior only, not Firestore-backed authority freshness.
- [x] 4.5 Record emulator check outcomes in `openspec/changes/consolidate-auth-authority/tasks.md` by checking off completed matrix tasks only after local emulator evidence passes. `npm run verify:auth-authority` passed 78/78 checks with callable, Firestore, and Storage emulator coverage; Storage warnings remain documented because stale/missing-profile claims can still authorize broader Storage access than canonical profiles.
...
- [x] 5.3 Run auth-authority emulator verification against Firebase emulators and capture pass/fail evidence for stale-token/session and role matrix scenarios. Evidence: `npm run verify:auth-authority` passed 78/78 checks after setting local `JAVA_HOME`/`Path` to JDK 21 for the command.
